...
with st.expander(f"Vulnerability Summary {ref_data_from}-2025", expanded=True, icon=":material/explore:"):
    ...
    count_by_month = df.groupby([df['datePublished'].dt.to_period('M'), 'Priority'], observed=True).size().reset_index(name='v')
    #count_by_month['cumulative_v'] = count_by_month.groupby('Priority')['v'].cumsum()
    total_by_priority = count_by_month.groupby('Priority', observed=True)['v'].sum().reset_index()
    ...
    with st.container():
        ...
        today_utc = pd.Timestamp.now(tz='UTC')
        # Calculate the last 12 complete months, ending in the previous month
        last_complete_month = today_utc - pd.DateOffset(months=1)
        end_period = last_complete_month.to_period('M')
        start_period = (last_complete_month - pd.DateOffset(months=11)).to_period('M')
        full_period_range = pd.period_range(start=start_period, end=end_period, freq='M')

        # Group all data by month and priority in one go
        all_monthly_counts = df.groupby([df['datePublished'].dt.to_period('M'), 'Priority'], observed=True).size().unstack(fill_value=0)
